...
//目标持仓.
map<string, int> targetPortfolio;
//未成交订单
...
public:
  ...
  TSimpleHandler(CThostFtdcTraderApi *pUserApiTrader, CThostFtdcMdApi *pUserApiMd,
                 CSimpleHandler *p) : m_pUserApiTrader(pUserApiTrader), m_pUserApiMd(pUserApiMd), p_C(p) {}
  ~TSimpleHandler() {}
      ...
      bool bIsLast)
  {
    ...
    if (pPosition != NULL)
    {
      ...
      cout << "Position of " << pPosition->InstrumentID << " is "
           ...
           << endl;
      TradingInstruments.insert(pPosition -> InstrumentID);
    }
    if (bIsLast)
    {
...
public:
  ...
  void ManageInstrument(string InstrumentID, int displace)
  {
    ...
    map<string, vector<Kline> >::iterator Kline_it = KlineMap.find(InstrumentID);
    if (Kline_it == KlineMap.end()) {
      cout << InstrumentID << " no price yet." << endl;
      return;
    }
    double LastPrice = KlineMap[InstrumentID].back().elements.back().ClosePrice;
    int HoldingPosition, TargetPosition;
    map<string, double>::iterator position_it = Portfolio.find(InstrumentID);
    if (position_it == Portfolio.end()) 
    {
      HoldingPosition = 0;
    } 
    else
    {
      HoldingPosition = position_it -> second;
    }
    map<string, int>::iterator it = targetPortfolio.find(InstrumentID);
    if (it == targetPortfolio.end()) 
    {
      TargetPosition = 0;
    }
    else
    ...
    if (TargetPosition != HoldingPosition)
    {
      if (TargetPosition >= 0 && HoldingPosition >= 0)
      {
        ...
        else if (TargetPosition < HoldingPosition)
        { /// Sell Close
          PlaceOrder(InstrumentID, THOST_FTDC_D_Sell,
                     THOST_FTDC_OF_Close, HoldingPosition - TargetPosition);
        }
      }
      else if (TargetPosition <= 0 && HoldingPosition <= 0)
      {
        ...
        { /// Buy Close
          PlaceOrder(InstrumentID, THOST_FTDC_D_Buy,
                     THOST_FTDC_OF_Close, TargetPosition - HoldingPosition);
        }
        else if (TargetPosition < HoldingPosition)
        { /// Sell Open
          PlaceOrder(InstrumentID, THOST_FTDC_D_Sell,
      ...
      { /// Buy Close then Buy Open
        PlaceOrder(InstrumentID, THOST_FTDC_D_Buy,
                   THOST_FTDC_OF_Close, -HoldingPosition);
        PlaceOrder(InstrumentID, THOST_FTDC_D_Buy,
                   THOST_FTDC_OF_Open, TargetPosition);
      }
      else if (TargetPosition < 0 && HoldingPosition > 0)
      { /// Sell Close then Sell Open
        PlaceOrder(InstrumentID, THOST_FTDC_D_Sell,
                   THOST_FTDC_OF_Close, HoldingPosition);
        PlaceOrder(InstrumentID, THOST_FTDC_D_Sell,
                   THOST_FTDC_OF_Open, -TargetPosition);
  ...
  void addTick2Kline(CThostFtdcDepthMarketDataField *tick)
  {
    ...
    else
    {
      ...
      //整分钟时执行交易.
      if (tick->UpdateTime[6] == '0' && tick->UpdateTime[7] == '0' && tick->UpdateMillisec == 0)
      {
  ...
  void loadTargetPortfolio() {
    map<string, string> mainInstrument;
    ifstream instrumentFile;
    instrumentFile.open("instruments");
    string productID, instrumentID;
    while ( instrumentFile >> productID)
    {
      instrumentFile >> instrumentID;
      mainInstrument[productID] = instrumentID;
      TradingInstruments.insert(instrumentID);
    }
    instrumentFile.close();

    ifstream File;
    File.open("tradingConfig");
    string portfolioFilename = "";
    string param;
    while (File >> param) 
    {
      if ( param.compare("tradingLevel") == 0) 
      {
        File >> tradingLevel;
      } 
      else if ( param.compare("portfolioFile") == 0)
      {
        File >> portfolioFilename;
      }
    }
    if ( portfolioFilename.compare("") == 0 )
    {
      cout << "Please set portfolioFile in tradingConfig" << endl;
    } 
    else 
    {
      ifstream portfolioFile;
      portfolioFile.open(portfolioFilename.c_str());
      string buf;
      //skip head.
      getline(portfolioFile, buf);
      while(getline(portfolioFile, buf, ','))// BloombergCode
      {
        string productCode, exchange, positionString, month;
        getline(portfolioFile, productCode, ',');
        getline(portfolioFile, exchange, ',');
        getline(portfolioFile, positionString, ',');
        getline(portfolioFile, month);
        double position = strtod(positionString.c_str(), NULL);
        int positionLots = trunc(position * tradingLevel / 1e6);
        targetPortfolio[ mainInstrument[ productCode ] ] = positionLots;
      }
    }
    cout << "target portfolio" << endl;
    for ( map<string, int>::iterator it = targetPortfolio.begin();
    it != targetPortfolio.end(); it ++ )
    {
